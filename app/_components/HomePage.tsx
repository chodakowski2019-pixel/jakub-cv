...
function LangSwitch({ lang }: { lang: Lang }) {
  const target = lang === "pl" ? "/en" : "/";
  const next = lang === "pl" ? "en" : "pl";
  const label = lang === "pl" ? "EN" : "PL";
  return (
    <a
      href={target}
      onClick={() => {
      ...
      className="text-xs font-semibold text-neutral-300 hover:text-white px-3 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.04] backdrop-blur-xl hover:bg-white/[0.08] hover:border-cyan-500/40 transition-all duration-300"
    >
      {label}
    </a>
  );
